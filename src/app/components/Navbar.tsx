      ...
      <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between px-5 sm:px-8 md:h-[88px] lg:px-8">
        ...
        <div className="hidden items-center gap-8 md:flex">
          {navLinks.map((link) => (
            <a
              ...
              className="text-[16px] font-normal capitalize leading-4 tracking-[2px] text-[#5b4137] transition hover:text-[#854d63] dark:text-[#dbc9c0] dark:hover:text-[#f0adc4]"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="hidden items-center gap-3 md:flex">
          <button
            type="button"
            onClick={toggleTheme}
            className="flex h-[52px] w-[52px] items-center justify-center rounded-full border border-[#e5e2e1] text-[#5b4137] transition hover:border-[#854d63]/40 hover:text-[#854d63] dark:border-white/15 dark:text-[#f8f1ec] dark:hover:border-[#f0adc4]/50 dark:hover:text-[#f0adc4]"
            ...
            <div className="flex flex-col gap-4">
              {navLinks.map((link) => (
                <a
                  ...
                  href={link.href}
                  onClick={(event) => scrollToSection(event, link.href)}
                  className="rounded-lg bg-[#fcf9f8] px-4 py-4 text-[16px] font-normal capitalize leading-5 tracking-[1px] text-[#5b4137] dark:bg-white/5 dark:text-[#dbc9c0]"
                >
                  {link.name}
                </a>
              ))}
