    ...
    <main className="overflow-hidden bg-[#fcf9f8] px-5 pb-20 pt-28 text-[#1c1b1b] dark:bg-[#13100f] dark:text-[#f8f1ec] sm:px-8 md:pt-36">
      ...
      >
        <div>
          ...
          <h1 className="mt-5 max-w-[620px] font-serif text-[42px] leading-[46px] text-[#1c1b1b] dark:text-[#f8f1ec] sm:text-[58px] sm:leading-[62px]">
            {t("services.pageTitleStart")}{" "}
            <span className="italic text-[#854d63] dark:text-[#f0adc4]">{t("services.pageTitleAccent")}</span>
          </h1>
        </div>
        ...
        <div className="flex flex-col justify-end gap-7">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[t("services.pageSignalOne"), t("services.pageSignalTwo")].map((signal) => (
              <div
                key={signal}
                className="rounded-lg border border-[#e4bfb2]/42 bg-white/64 p-4 text-[13px] font-semibold uppercase leading-5 tracking-[1.5px] text-[#854d63] shadow-[0_14px_38px_rgba(91,65,55,0.05)] dark:border-white/10 dark:bg-white/5 dark:text-[#f0adc4]"
              >
                {signal}
              </div>
            ))}
            ...
            >
              ...
              >
                <span className="flex items-start justify-between gap-6">
                  ...
                  <span className="text-[12px] font-semibold uppercase tracking-[2px] text-[#8d7b72] dark:text-[#cdb9ae]">
                    0{index + 1}
                  </span>
                </span>

                <span className="mt-10 block">
                  <span className="block font-serif text-[34px] leading-[38px] text-[#1c1b1b] dark:text-[#f8f1ec]">
                    {service.title}{" "}
                    <span className={`italic ${accent.text} dark:text-[#f0adc4]`}>{service.accent}</span>
                  </span>
                  <span className="mt-5 block max-w-[620px] text-[16px] leading-7 text-[#5b4137] dark:text-[#dbc9c0]">
                    {service.presentation ?? service.detailIntro}
                  </span>
                </span>

                <span className="mt-8 grid gap-3">
                  <span className="text-[11px] font-semibold uppercase tracking-[2px] text-[#854d63] dark:text-[#f0adc4]">
                    {t("services.includedPreview")}
                  </span>
                  {previewItems.map((item) => (
                    <span key={item} className="flex items-start gap-3 text-sm leading-6 text-[#5b4137] dark:text-[#dbc9c0]">
                      <CheckCircleIcon className="mt-0.5 size-5 shrink-0 text-[#854d63] dark:text-[#f0adc4]" />
                      <span>{item}</span>
                    </span>
                  ))}
                </span>

                <span className="mt-9 flex items-center justify-between gap-5 border-t border-[#e4bfb2]/42 pt-5 dark:border-white/10">
                  <span>
                    <span className="block text-[28px] font-semibold leading-none text-[#1c1b1b] dark:text-[#f8f1ec]">
                      {service.metricValue}
                    </span>
                    <span className="mt-2 block max-w-[260px] text-[12px] leading-5 text-[#6d625d] dark:text-[#cdb9ae]">
                      {service.metricLabel}
                    </span>
                  </span>
      ...
      <motion.section
        ...
        transition={{ duration: 0.48, ease: [0.22, 1, 0.36, 1] }}
        className="mx-auto mt-16 grid max-w-[1180px] gap-8 rounded-lg border border-[#e4bfb2]/38 bg-white p-7 shadow-[0_24px_70px_rgba(91,65,55,0.08)] dark:border-white/10 dark:bg-[#171111] sm:p-10 lg:grid-cols-[1fr_auto] lg:items-center"
      >
        <div>
          <p className="text-[12px] font-semibold uppercase tracking-[3px] text-[#854d63] dark:text-[#f0adc4]">
            {t("services.pageCtaEyebrow")}
          </p>
          <h2 className="mt-4 max-w-[720px] font-serif text-[34px] leading-[38px] text-[#1c1b1b] dark:text-[#f8f1ec] sm:text-[44px] sm:leading-[48px]">
            {t("services.pageCtaTitle")}
          </h2>
          <p className="mt-5 max-w-[720px] text-[16px] leading-7 text-[#5b4137] dark:text-[#dbc9c0]">
