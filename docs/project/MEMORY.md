...
- Public interactive controls use a 48 CSS pixel minimum touch target on mobile and desktop. The HTML shell links `/site.webmanifest` and a 180x180 Apple touch icon; the manifest reuses dedicated 192x192 and 512x512 branded icons.
- `labs/carole-bento-lab/` and `plans/` are local-only design workspaces. They stay gitignored on `dev` and `main`, are not imported by the primary Vite application, and are never deployed to production.
